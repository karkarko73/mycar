    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <form action="{{ url("user/personalaccount/$data->id/edit") }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="password" class="form-control" id="inputPassword" name="new_password"
                            placeholder="Enter If You Want To Change New Passowrd">
                    </div>
                </div>
                ...
                <a href="{{ url('user/personalaccount/'.auth()->user()->id.'/show') }}"
...
{{-- <td class="text-center">
    <a href="{{ url("admin/user/$data->id/edit") }}" class="btn btn-primary mr-3"><i class="fas fa-edit">Edit
    User</i></a>
<a href="{{ url("admin/user/$data->id/delete") }}"
    onclick="return confirm('Warning(Not to delete) : That can effect the products that you post in this city')"
    class="btn btn-danger"><i class="fas fa-trash-alt">Delete User</i></a>
</td> --}}
